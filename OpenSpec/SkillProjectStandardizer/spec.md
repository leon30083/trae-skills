...
SkillProjectStandardizer 的规范需明确如何让标准化后的项目可被外部代理使用：

### 模式一：API-as-Tool（简约封装）
- 输出：在脚手架中包含 `api/` 目录模板，将规则路由的受控脚本封装为 REST API。
- 工具：提供 OpenAPI/JSON Schema 模板，适配 Gemini 函数调用；标注延迟与黑盒故障风险。

### 模式二：MCP-as-Bus（中介协议）
- 输出：在脚手架中包含 `mcp/` 目录模板，将白名单脚本注册为 MCP 工具。
- 约束：参数与路径白名单、交互确认；只读接口与受控执行接口清晰区分。
- 指南：提供 VS Code 客户端配置样例与权限说明。

### 模式三：Agent-as-Orchestrator（ToT 编排）
- 输出：在脚手架中包含 `orchestrator/` 模板（LangChain/LightAgent），定义工具清单与编排策略。
- 策略：失败回溯、替代路径、分支选择与评估报告汇总。
