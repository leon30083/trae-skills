...
## 互操作性章节（依据《AI Skills 互操作性架构设计.md》）
为适配多模型代理（Gemini/Claude/ToT 编排），Doc2Skill 的产物需要支持以下三种集成模式：

### 模式一：API-as-Tool（简约封装）
- 目标：将生成的 Skill 的执行封装为一个 REST API，供 Gemini 的函数调用工具使用。
- 要点：
  - 在产物中生成 `api/` 目录的接口骨架示例（无服务器优先，如 Cloud Run/Lambda）。
  - 提供 OpenAPI/JSON Schema 示例，定义最小输入（如 prompt、payload 路径）与输出（结果 URL/JSON）。
  - 文档标注高延迟与黑盒故障的权衡。

### 模式二：MCP-as-Bus（中介协议）
- 目标：将 Skill 操作暴露为 MCP 工具，供任意支持 MCP 的客户端调用。
- 要点：
  - 在产物中生成 `mcp/` 目录的工具描述与最小 server 示例（只读+白名单执行）。
  - 约束：参数白名单、路径白名单、交互确认；仅读/受控执行接口。
  - 文档说明与 VS Code MCP 客户端的连接方法与权限配置。

### 模式三：Agent-as-Orchestrator（ToT 编排）
- 目标：为 ToT 元代理提供可编排的模块接口，允许分支/回溯/替代方案。
- 要点：
  - 在产物中生成 `orchestrator/` 目录示例（LangChain/LightAgent），定义工具 A（分析）与工具 B（Skill 调用）。
  - 提供失败回溯策略与替代路径（例如 Skill 失败时切换另一模型分析）。
